...
function Recebimentos() {
  ...
  async function loadRecebimentos() {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Erro ao carregar recebimentos.");
    } finally {
      setLoading(false);
    }
  }
  ...
  function openEditModal(recebimento) {
    setIsEditing(true);
    setCurrentId(recebimento.id);
    setContratoId(recebimento.contrato_id || "");
    setDataVencimento(recebimento.data_vencimento || "");
    setDataPagamento(recebimento.data_pagamento || "");
    setValorCobrado(recebimento.valor_cobrado || "");
    setValorRecebido(recebimento.valor_recebido || "");
  ...
  async function handleSubmit(e) {
    ...
    const payload = {
      ...
      valor_recebido: valor_recebido ? Number(valor_recebido) : null,
      status,
    };
    ...
    try {
      ...
      setError(err.response?.data?.message || "Erro ao salvar recebimento.");
    } finally {
  ...
  async function handleDelete(id) {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Erro ao excluir recebimento.");
    }
  }
  ...
  function getContratoLabel(id) {
    ...
    return contrato ? `Contrato #${contrato.id}` : `ID ${id}`;
  }
    ...
    <div className="recebimentos-page min-vh-100">
      ...
      <nav className="navbar navbar-expand-lg navbar-light bg-white shadow-sm">
        <div className="container">
          ...
          <div className="d-flex gap-2 flex-wrap">
            ...
            <button className="btn btn-outline-primary btn-sm" onClick={() => navigate("/contratos")}>
              Contratos
            </button>
      ...
      <div className="container py-4">
        <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
          <div>
            <h2 className="fw-bold mb-1">Recebimentos</h2>
            <p className="text-muted mb-0">Gerencie os recebimentos dos contratos</p>
          </div>
                      ...
                      >
                        ...
                        {contratos.map((item) => (
                          <option key={item.id} value={item.id}>
                            Contrato #{item.id} - Valor: {formatCurrency(item.valor)}
                          </option>
                        ))}
                      </select>
                    </div>
